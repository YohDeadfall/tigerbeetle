...
fn expect_json_file_equals(
    ...
    var token_stream = std.json.TokenStream.init(out_file_filtered.items);
    const options = .{ .allocator = arena.allocator() };

    // Fails to compile in std.json.parse with the default
    // branch quota.
    @setEvalBranchQuota(5000);
    var parsed = try std.json.parse(T, &token_stream, options);
    defer std.json.parseFree(T, parsed, options);
